...
interface KPICardProps {
  ...
  formula?: string;
  formulaNote?: string;
  formulaPosition?: "right" | "left" | "top" | "bottom";
}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <p
          className={cn(
            "text-xs font-medium uppercase tracking-wider flex items-center",
            highlight ? "text-zinc-400" : alert ? "text-rose-500" : "text-zinc-400"
          )}
        >
          {label}
          {formula && (
            <FormulaTooltip formula={formula} note={formulaNote} position={formulaPosition} />
          )}
        </p>
        {Icon && (
          <Icon
