          ...
          {repStat && (
            <div className="grid grid-cols-4 gap-3">
              <div className="bg-zinc-50 rounded-lg p-3">
                <p className="text-[10px] text-zinc-400 uppercase tracking-wide flex items-center">
                  Total Leads
                  <FormulaTooltip formula={`COUNT(all leads)\nWHERE assigned_to\n  = this rep`} note="Includes all statuses: active, delivered, and lost." position="bottom" />
                </p>
                <p className="text-base font-bold text-zinc-900 mt-0.5">{repStat.totalLeads}</p>
              </div>
              <div className="bg-zinc-50 rounded-lg p-3">
                <p className="text-[10px] text-zinc-400 uppercase tracking-wide flex items-center">
                  Delivered
                  <FormulaTooltip formula={`COUNT(leads)\nWHERE status\n  = "delivered"`} note="Deals closed and handed over to the customer." position="bottom" />
                </p>
                <p className="text-base font-bold text-zinc-900 mt-0.5">{repStat.delivered}</p>
              </div>
              <div className="bg-zinc-50 rounded-lg p-3">
                <p className="text-[10px] text-zinc-400 uppercase tracking-wide flex items-center">
                  Revenue
                  <FormulaTooltip formula={`SUM(deal_value)\nWHERE status\n  = "delivered"`} note="Total revenue this rep generated from closed deals." position="bottom" />
                </p>
                <p className="text-base font-bold text-zinc-900 mt-0.5">{formatCurrency(repStat.revenue)}</p>
              </div>
              <div className="bg-zinc-50 rounded-lg p-3">
                <p className="text-[10px] text-zinc-400 uppercase tracking-wide flex items-center">
                  Conversion
                  <FormulaTooltip formula={`delivered ÷ totalLeads\n  × 100`} note="What % of assigned leads were converted to a delivery." position="left" />
                </p>
                <p className="text-base font-bold text-zinc-900 mt-0.5">{repStat.conversionRate}%</p>
              </div>
            </div>
          )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-50 bg-zinc-50/50">
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Rank</th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Name</th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Branch</th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Role</th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Leads
                      <FormulaTooltip formula={`COUNT(all leads)\nWHERE assigned_to\n  = this rep`} note="Total leads ever assigned to this rep under the selected filter." position="bottom" />
                    </th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Delivered
                      <FormulaTooltip formula={`COUNT(leads)\nWHERE status\n  = "delivered"`} note="Number of deals the rep successfully closed." position="bottom" />
                    </th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Revenue
                      <FormulaTooltip formula={`SUM(deal_value)\nWHERE status\n  = "delivered"`} note="Total revenue from delivered deals. Leaderboard is sorted by this column." position="bottom" />
                    </th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Conv %
                      <FormulaTooltip formula={`delivered ÷ totalLeads\n  × 100`} note="Conversion rate: how many of the rep's leads resulted in a delivery." position="bottom" />
                    </th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Avg Deal
                      <FormulaTooltip formula={`revenue ÷ delivered`} note="Average deal size per closed unit. Empty if no deliveries." position="bottom" />
                    </th>
                    <th className="px-4 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-50">
                  {reps.map((rep, i) => (
