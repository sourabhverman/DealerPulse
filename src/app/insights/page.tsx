              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-zinc-100 bg-zinc-50/50">
                    <th className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Branch</th>
                    <th className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Target</th>
                    <th className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Delivered
                      <FormulaTooltip formula={`COUNT(leads)\nWHERE status = "delivered"\n  AND month = Dec 2025`} note="Units actually handed over to customers this month." position="bottom" />
                    </th>
                    <th className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      In Pipeline
                      <FormulaTooltip formula={`COUNT(active leads)\n= new + contacted\n  + test_drive\n  + negotiation\n  + order_placed`} note="Active leads still in progress — not yet delivered or lost." position="bottom" />
                    </th>
                    <th className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Projected
                      <FormulaTooltip formula={`actualUnits\n+ ROUND(pipelineUnits × 60%)`} note="Assumes 60% of current pipeline converts before month end." position="bottom" />
                    </th>
                    <th className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
                      Gap
                      <FormulaTooltip formula={`targetUnits − projectedUnits`} note="Negative = ahead of target. Positive = still short of target." position="bottom" />
                    </th>
                    <th className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Status</th>
                  </tr>
                </thead>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <section>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 flex items-center">
                Cold Leads — Act Now
                <FormulaTooltip formula={`daysSinceActivity =\n  today − last_activity_at\nShown if ≥ 7 days`} note="Days count in red = 14d+ (urgent), amber = 7–13d. Value = lead's deal_value." position="right" />
              </h2>
            </div>
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
          ...
          <section>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 flex items-center">
                Top Performers
                <FormulaTooltip formula={`Revenue = SUM(deal_value)\n  WHERE status = "delivered"\nRanked by revenue DESC`} note="Delivered count = leads the rep personally closed. Revenue = sum of their deal values." position="right" />
              </h2>
            </div>
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
              <div className="divide-y divide-zinc-50">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-emerald-600 mt-0.5 flex items-center">
                    <span className="flex items-center">
                      ...
                      <FormulaTooltip formula={`deliveredFromSource\n÷ totalFromSource × 100`} note="Of all leads from this channel, how many were ultimately delivered." position="top" />
                    </span>
